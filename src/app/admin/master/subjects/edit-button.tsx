...
export function EditSubjectButton({ subject }: Props) {
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[450px] rounded-2xl border-0 shadow-2xl p-0 overflow-hidden">
        <div className="bg-gradient-to-r from-blue-50 to-white p-6 border-b border-slate-100">
          <DialogHeader>
            <DialogTitle className="text-2xl font-extrabold text-slate-800">Edit Mata Pelajaran</DialogTitle>
        ...
        <form action={handleSubmit} className="p-6 space-y-6 bg-white">
          ...
          <div className="space-y-2">
            <Label htmlFor="code" className="text-slate-700 font-bold">Kode Mapel</Label>
            <Input 
              id="code" 
              name="code" 
              defaultValue={subject.code}
              className="h-12 rounded-xl border-slate-200 bg-slate-50 focus-visible:ring-blue-500 focus-visible:bg-white transition-all font-medium text-slate-800 uppercase"
            />
          </div>
