...
export function DeleteSubjectButton({ id, name, schedulesCount }: Props) {
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="sm:max-w-[400px] rounded-2xl border-0 shadow-2xl p-0 overflow-hidden">
        <div className="bg-red-50 p-6 border-b border-red-100 flex flex-col items-center text-center">
          ...
          <DialogHeader>
            <DialogTitle className="text-xl font-extrabold text-red-900">Hapus Mapel?</DialogTitle>
            <DialogDescription className="text-red-700/80 font-medium mt-2">
              {hasSchedules 
                ? `Mata Pelajaran ${name} sedang digunakan di ${schedulesCount} jadwal kelas. Hapus jadwal terlebih dahulu.`
                : `Anda yakin ingin menghapus mata pelajaran ${name}?`}
            </DialogDescription>
          </DialogHeader>
        </div>
